...
export function OrderConfirmation({ orderId }: { orderId: string }) {
  ...
  return (
    <main className="relative mx-auto max-w-4xl overflow-hidden px-4 py-10">
      <Confetti />
      <section className="relative rounded-[2rem] bg-white p-6 text-center shadow-xl sm:p-10">
...
function Confetti() {
  ...
  return <div className="pointer-events-none absolute inset-x-0 top-0 h-40 overflow-hidden">{Array.from({length: 22}, (_, index) => <span key={index} className={`confetti-piece absolute size-2 rounded-sm ${colors[index % colors.length]}`} style={{ left: `${(index * 43) % 100}%`, animationDelay: `${(index % 7) * 0.09}s` }} />)}</div>;
}
