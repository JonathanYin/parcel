...
export function CheckoutForm() {
  ...
  const { cart, createOrder, hydrated } = useStore();
  ...
  const field = (key: keyof ShippingAddress, label: string, type = "text") => (
    <label className="block text-sm font-bold text-slate-700">{label}<input required type={type} value={address[key]} onChange={(event) => setAddress({ ...address, [key]: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2.5 font-normal outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" /></label>
  );
